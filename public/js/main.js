...
const showTerminalResult = (terminal) => {
  ...
  const decorationTimeout = setTimeout(() => {
    ...
    const terminalConfetti = confetti.create(terminal.querySelector('.terminal__confetti'), {
      resize: true,
    });
    terminalConfetti({
      angle: 360,
...
const handleVideosRender = () => {
  videoTags.forEach((video) => {
    ...
    const isHightResolution = window.matchMedia(
      '(-webkit-min-device-pixel-ratio: 1.25), (-o-min-device-pixel-ratio: 5/4), (min-resolution: 120dpi)'
    ).matches;
    ...
    video.innerHTML = `
    <source src="/video/${
      video.dataset.videoTag
    }__${videoSizeSuffix()}${videoResolutionSuffix()}.webm" type="video/webm">
    <source src="/video/${
      video.dataset.videoTag
    }__${videoSizeSuffix()}${videoResolutionSuffix()}.mp4" type="video/mp4">
    `;
